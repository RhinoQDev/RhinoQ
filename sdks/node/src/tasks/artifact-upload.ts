...
export class ArtifactUploadService {
  ...
  async create(request: CreateArtifactUploadRequest): Promise<{ session: ArtifactUploadSession; plan: MultipartPlan }> {
    ...
    const id = randomUUID(), artifactId = request.artifactId?.trim() || `artifact-${createHash('sha256').update(`${tenantId}\0${ownerId}\0${name}\0${request.sizeBytes}`).digest('hex').slice(0, 32)}`;
  ...
  async resume(id:string,ownerId:string,tenantId:string):Promise<ArtifactUploadSession>{let session=await this.store.getForOwner(id,ownerId,tenantId);if(session.state==='uncertain')return session;ensureUploading(session);const providerParts=await this.direct.listParts({uploadId:session.providerUploadId,reference:session.reference});const parts=providerParts.sort((a,b)=>a.partNumber-b.partNumber);if(JSON.stringify(parts)!==JSON.stringify(session.parts))session=await this.store.save({...session,parts},session.version);return session;}
  ...
  async complete(id: string, ownerId: string, tenantId: string, expectedVersion: number): Promise<{ session: ArtifactUploadSession; artifact?: TaskArtifact }> {
    ...
    if (session.state === 'uncertain') {
      ...
      try {
        ...
        const artifact = session.taskId && this.register ? await this.registerCompleted(session) : undefined;
