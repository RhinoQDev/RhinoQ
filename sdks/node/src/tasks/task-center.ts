...
function verificationText(task){return task.state==='uncertain'?'Needs confirmation before retry':'No verification issue is recorded'}
...
function needsAttention(task){const c=itemCounts(task),cancel=task.cancellation&&task.cancellation.status;return task.state==='failed'||task.state==='uncertain'||(c.failed>0&&c.succeeded>0)||cancel==='too_late'||cancel==='cannot_cancel_safely'}
