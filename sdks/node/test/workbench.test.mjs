...
test('the page is self-contained: no external origin is referenced', async () => {
  ...
  assert.match(html, /let active = initialParams\.get\('view'\) \|\| 'attention'/);
  assert.match(html, /id="listPager"/);
  assert.match(html, /listPageSize = 10/);
  assert.match(html, /function moveListPage/);
  assert.match(html, /Previous Workbench task page/);
  assert.match(html, /Next Workbench task page/);
  assert.match(html, /What this means/);
  assert.match(html, /Next action/);
  assert.match(html, /Nothing needs attention/);
  assert.match(html, /href="\/task-center"/);
  assert.match(html, />My activity<\/a>/);
  assert.match(html, />Overview<\/a>/);
  assert.doesNotMatch(html, /actions enabled/);
  assert.match(html, /id="detailDrawer"/);
  assert.match(html, /id="detailBackdrop"/);
  assert.match(html, /id="closeDetail"/);
