...
`0.1.0-beta.21` is the latest verified public prerelease. `0.1.0-beta.10` was
...
2. Run from a clean checkout:
   ...
   npm run pack:check
   cd ../..
   node .github/scripts/verify-release-matrix.mjs v0.1.0-beta.21
   ```
...
5. Verify the resulting state independently:
   ...
   ```bash
   npm view @rhinoq/node@0.1.0-beta.21 version dist.integrity dist.attestations
   npm view rhinoq@0.1.0-beta.21 version dist.integrity dist.attestations
   npm dist-tag ls @rhinoq/node
   npm dist-tag ls rhinoq
   gh release view v0.1.0-beta.21
   ```
   ...
   ```bash
   npm dist-tag add @rhinoq/node@0.1.0-beta.21 latest
   npm dist-tag add rhinoq@0.1.0-beta.21 latest
   ```
...
6. Verify the keyless checksum bundle:
   ...
   cosign verify-blob checksums.txt \
     ...
     --certificate-identity "https://github.com/madebyduy/RhinoQ/.github/workflows/release.yml@refs/tags/v0.1.0-beta.21" \
