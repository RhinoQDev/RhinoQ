...
Pin the exact beta.21 package after the release workflow succeeds; before that,
install the beta.21 tarball built from this checkout as described in
...
[beta.21 release](https://github.com/madebyduy/RhinoQ/releases/tag/v0.1.0-beta.21),
...
| npm installs an unexpected version | during public beta both `latest` and `next` should resolve to the verified release | pin an exact version, for example `@rhinoq/node@0.1.0-beta.21`, and report the stale tag |
