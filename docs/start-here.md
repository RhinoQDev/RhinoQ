...
Pin the exact beta.13 package after the release workflow succeeds; before that,
install the beta.13 tarball built from this checkout as described in
...
[beta.13 release](https://github.com/madebyduy/RhinoQ/releases/tag/v0.1.0-beta.13),
...
| npm installs an unexpected version | prereleases use `next` and `latest` is not moved by the prerelease workflow | pin an exact version, for example `@rhinoq/node@0.1.0-beta.13` after the release succeeds |
