...
```bash
npm install @rhinoq/node@0.1.0-beta.16 pg
```

Why: `pg` is a peer dependency and lets RhinoQ reuse the application's pool.
Pin the exact beta.16 package after the release workflow succeeds; before that,
install the beta.16 tarball built from this checkout as described in
...
[beta.16 release](https://github.com/madebyduy/RhinoQ/releases/tag/v0.1.0-beta.16),
...
| npm installs an unexpected version | during public beta both `latest` and `next` should resolve to the verified release | pin an exact version, for example `@rhinoq/node@0.1.0-beta.16`, and report the stale tag |
