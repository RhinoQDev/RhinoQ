...
> Status: public prerelease. Pin the exact published `0.1.0-beta.12` version;
...
The beta.12 prerelease exports `TaskStore`, a browser external
...
| `npm run pack` | removes earlier archives, rebuilds and creates the archive | `rhinoq-node-0.1.0-beta.12.tgz` |
...
```bash
npm install /path/to/rhinoq/sdks/node/rhinoq-node-0.1.0-beta.12.tgz pg
```
...
published. The tagged beta.12 prerelease is verified on npm and GitHub and
...
- Every published version is a prerelease. The beta.12 release workflow uses
