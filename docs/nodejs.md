...
> Status: public prerelease. Pin the exact published `0.1.0-beta.26` version;
...
The beta.26 prerelease exports `TaskStore`, a browser external
...
| `npm run pack` | removes earlier archives, rebuilds and creates the archive | `rhinoq-node-0.1.0-beta.26.tgz` |
...
```bash
npm install /path/to/rhinoq/sdks/node/rhinoq-node-0.1.0-beta.26.tgz pg
```

Windows PowerShell example:

```powershell
npm install C:\src\rhinoq\sdks\node\rhinoq-node-0.1.0-beta.26.tgz pg
```
...
published. The tagged beta.26 prerelease is verified on npm and GitHub and
...
- Every published version is a prerelease. The beta.26 release workflow uses
