...
The beta.18 package contains the current `verify` commands. An older tarball
answers `FAIL verify requires 'add <rule-name>'`; if that is what you see,
install the exact beta.18 package or tarball.
...
The published beta.18 package and GitHub archive contain this Task-only path.
