...
The beta.20 package contains the current `verify` commands. An older tarball
answers `FAIL verify requires 'add <rule-name>'`; if that is what you see,
install the exact beta.20 package or tarball.
...
The published beta.20 package and GitHub archive contain this Task-only path.
