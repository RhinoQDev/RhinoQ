...
The beta.24 package contains the current `verify` commands. An older tarball
answers `FAIL verify requires 'add <rule-name>'`; if that is what you see,
install the exact beta.24 package or tarball.
...
```bash
npm install /absolute/path/to/rhinoq-node-0.1.0-beta.24.tgz pg
RHINOQ_DATABASE_URL='postgres://...' npx rhinoq-task
```

The published beta.24 package and GitHub archive contain this Task-only path.
