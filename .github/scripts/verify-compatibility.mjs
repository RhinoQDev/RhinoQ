...
const [matrix, workflow, goMod, lock] = await Promise.all([
  read('docs/compatibility-matrix.md'), read('.github/workflows/ci.yml'), read('go.mod'),
  json('examples/fanout-bullmq/package-lock.json'),
]);
const bullmq = lock.packages?.['node_modules/bullmq']?.version;
...
const required = [
  ...
  ['BullMQ', bullmq && matrix.includes(`| BullMQ | ${bullmq} |`)],
